...
class Complex {
	 ...
	 */
	final public function __construct(float $real, float $imaginary) {
		$this->x = $real;
		$this->y = $imaginary;
	}
	...
	public static function parse(Complex|Rational|int|float|string $value): Complex {
		...
		if ($valid === 1) {
			...
			try {
				...
				$imaginaryPart = self::parseReal($imaginary);
			}
		} else {
