...
namespace Tests\MathParser\Parsing;

use MathParser\Exceptions\{ParenthesisMismatchException, SyntaxErrorException};
use MathParser\Interpreting\TreePrinter;
use MathParser\Lexing\{StdMathLexer, Token, TokenType};
use MathParser\Parsing\Nodes\{ConstantNode, ExpressionNode, FunctionNode, IntegerNode, NumberNode, VariableNode};
...
class StdMathParserTest extends TestCase {
	...
	public function testNonSimplifyingParser() {
		...
		$node = $this->parser->parse('3+5');
		$shouldBe = new ExpressionNode(new IntegerNode(3), '+', new IntegerNode(5));
		$this->assertNodesEqual($node, $shouldBe);
		...
		$node = $this->parser->parse('-3');
		$shouldBe = new ExpressionNode(new IntegerNode(3), '-', null);
		$this->assertNodesEqual($node, $shouldBe);
	}
